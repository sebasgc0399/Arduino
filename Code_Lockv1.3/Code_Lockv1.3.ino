...
//Variables para La doble validacion (RFID con Clave)
boolean VRFID = false;
boolean VCODE = false;
int intentos = 0;
...
void loop() {
  ...
  //Validador de doble seguridad
  if(VRFID == true && VCODE == true){
    VCODE = false;
    VRFID = false;
    lcd.clear();
    Rele();
    }else if (VRFID == false && VCODE == true){
      intentos+=1;
      }else if (VRFID == true && VCODE == false){
        intentos+=1;
      }
  ...
  if (comparaUID(LecturaUID, Usuario1)) { // llama a funcion comparaUID con Usuario1
    ...
    lcd.setCursor(0, 1);
    lcd.print("Sebastian");
    VRFID = true;      //Limpiar la pantalla LCD
  }
